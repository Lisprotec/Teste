...
function evaluateRule(data, ut, rule) {
  ...
  if ('riskDE' in rule) details.push(checkMetric('Efetivo em locais de risco D/E', data.riskDEOccupancy, rule.riskDE, '≤', ''));
  ...
  if (rule.requireIndependentD) details.push(checkBoolean('Saída independente direta ao exterior para locais de risco D', data.independentExitD === 'yes'));
  if (rule.requireIndependentE) details.push(checkBoolean('Saída independente direta ao exterior para locais de risco E', data.independentExitE === 'yes'));
